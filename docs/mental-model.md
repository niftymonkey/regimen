...
- **The loop is see, act, validate, a cycle and not a line, run in conversation with your agent and on demand.**
  - Beat 1, see: you ask ("how is this going?", "what keeps tripping up my refactors?"), your agent reads Feedback, and it surfaces a pattern you would otherwise only feel.
  - Beat 2, act: you pull a lever, a quick steer or a durable change like a skill (Guidance) or a gate (Enforcement). This is your move, not Regimen's.
  - Beat 3, validate: you ask again, and Feedback shows you whether what you built actually changed anything.
  ...
  - **On demand, never unprompted:** the trigger is always yours, but the answer can still be something you did not know to ask about. Regimen does not volunteer patterns mid-conversation, which would be the Clippy failure mode. If you ever want a nudge without asking, that is itself an opt-in Guidance skill you install, never a default.
...
- **The scale you observe is independent of how you respond.** Looking at one conversation or at the trend across many is only about the window you observe; it does not dictate the move. A durable lever can come from a pattern you spot in a single long conversation (restart the harness to pick it up if needed), and a light in-flight steer can be informed by the trend.
  - **Why:** an earlier framing chained the two, a quick steer only at the conversation scale and a durable lever only at the cross-conversation scale. That is wrong: you can and often should build something lasting the moment a recurring problem shows up in one conversation. The scales differ only in which patterns are visible, since a single session cannot show a cross-session trend and the trend cannot show this exact moment.
...
- **Regimen ships a few operator skills, and they are not lever content.** An operator skill is a prompt Regimen bundles so your agent can drive Regimen's own loop in conversation. The two Feedback skills are the operator skills for the see beat (read the evidence, read the judgment). The respond-step helper, once it exists, is the operator skill for the act beat on the Guidance side: given a pattern Feedback surfaced, it helps your agent find or build the skill that addresses it. These operate the instrument; they never supply the disciplines. The respond-step helper still only helps you locate or author your own engineering skill, so "Regimen points, does not supply" holds even here.
  - **Why:** "it is a skill" does not make something Guidance. A skill is a form; what decides the category is what the skill operates, Regimen's own loop (operator skill, shipped) versus your engineering (lever content, external). Collapsing the two would make Guidance mean "any skill anywhere," and the lever would stop cutting anything. The operator skills and the lever content are different species that happen to share a file shape. This is why the respond-step helper is the same kind of artifact as the two Feedback skills (Regimen's own, bundled), even though it serves the Guidance act rather than the see beat.
  - **Rules out:** counting the bundled Feedback skills, or the respond-step helper, as instances of the Guidance lever; and treating the respond-step helper as Regimen shipping guidance content.

- **Regimen installs as one system, composed from many sources.** There is a single install. `regimen install` sets up Regimen, and internally it pulls from several places at once: today the capture hook, the loader daemon, and the operator skills that Feedback installs, plus the reference gate wiring on the Enforcement side, and tomorrow whatever a new lever contributes. It is one thing being installed, not three; the composition is internal, and outward copy should not present it as a separate install per lever.
  - **Why:** the unit is Regimen, the way a single tool installs even though it lays down several files. This matches the implementation, where one verb already composes the pieces in-process, and it is future-proof: a new installable helper in any lever (an Enforcement prompt, a second Guidance asset) is absorbed with no change to the install story. The guardrail that keeps it honest: install sets up Regimen's own machinery, never the levers' content. It does not install your skill catalog or a gate library as if those were the product, so "Regimen points, does not supply" holds at install time too.
  - **Rules out:** an install section that enumerates a separate install per lever, or any wording that implies Guidance or Enforcement is installed as its own product alongside Feedback. The breadth of each lever (the next point) is described apart from what install does, so "install" is never read as "installs the whole category."
...
- **Local by construction.** Everything Regimen captures and stores lives on the engineer's machine. Regimen collects nothing for its authors and exfiltrates none of your captured telemetry. This is a claim about your data, not a promise of network silence. The normal network calls still happen: the judgment layer's LLM call goes to your own already-configured model provider, and the Guidance find-branch, when you use it, searches a public skill directory. Neither carries your captured conversations, and neither goes to Regimen.
  - **Why:** "observability" plus "telemetry on your conversations" can read as surveillance, and it is the opposite. The data is for you and stays on your machine. The point is that Regimen does not phone home with what it observes, not that nothing touches the network (your agent already calls a model provider over the network every turn).
