...
| Drift-gate denials | Count of tool calls a hook denied | How often deterministic discipline gates actually caught something | On Claude, capture the `PermissionDenied` hook event, which fires whenever a gate denies a tool call. Cross-harness that event does not exist; a denial is instead emitted by the denying hook itself, which the convention of Shepherd hooks emitting observability events already provides. Reads zero until discipline hooks (Shepherd-style) exist to produce denials. |
