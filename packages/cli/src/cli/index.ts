...
function installHarness(
  ...
  const code = runPillars(harness, dir, dryRun, steps, daemon);
...
Lifecycle:
  install [--all | --harnesses <h...>]   install Regimen (capture, skills) for the harness(es)
...
Flags:
  ...
  --judge-via <api|cli|agent>     assess: force the judge backend (agent = the emit/record flow); REGIMEN_JUDGE_VIA=<api|cli> sets it for every run
