...
The monorepo's root `./install.sh` composes this instrument through `regimen install`, which lays down the `enforcement-respond` skill (it wires no gates). The authored-gate wiring path is a library function the skill calls when the engineer confirms a gate, not a standalone command.
