...
The monorepo's root `./install.sh` composes this instrument through `regimen install`, which lays down the `regimen-guidance` skill. There is nothing else to wire: an advisory move is the engineer's own, enacted on demand by the skill, not at install time.
