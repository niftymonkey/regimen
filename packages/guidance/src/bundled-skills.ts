 ...
 * `<home>/<skillsSubdir>/<name>/` directory. `regimen-guidance` is the act-beat
 ...
 * `BUNDLED_SKILLS` and Enforcement's bundles `regimen-enforcement`. Adding another
 * Guidance skill is one entry here, not a new code path.
 */
export const BUNDLED_SKILLS = ["regimen-guidance"] as const;
