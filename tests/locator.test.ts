/**
 * The InstrumentLocator. Tests build a temp fixture tree: a fake parent dir
 * holding sibling clone dirs (`regimen`, `regimen-feedback`,
 * `regimen-enforcement`), each with a stub `src/cli/index.ts`. The hub clone
 * root and the environment are injected so the named-sibling convention and the
 * override chain are exercised against real files with a controlled env and no
 * mocking. Dependency category 2: filesystem + env only.
 */
...
/**
 * Build a parent dir with the named sibling clones present (by default both),
 * each carrying a stub `src/cli/index.ts`, and return the parent plus the hub
 * clone root inside it. Pass a subset of names to omit a clone for miss tests.
 */
function withSiblingTree(
  fn: (tree: { parent: string; hubCloneRoot: string }) => void,
  present: ReadonlyArray<string> = ["regimen-feedback", "regimen-enforcement"],
): void {
  const parent = mkdtempSync(join(tmpdir(), "regimen-hub-locator-"));
  try {
    const makeClone = (name: string): string => {
      const root = join(parent, name);
      mkdirSync(join(root, "src", "cli"), { recursive: true });
      writeFileSync(join(root, "src", "cli", "index.ts"), "// stub entry\n");
      return root;
    };
    const hubCloneRoot = makeClone("regimen");
    for (const name of present) makeClone(name);
...
test("the named-sibling convention resolves both instruments from the hub clone root", () => {
  withSiblingTree(({ parent, hubCloneRoot }) => {
    ...
    expect(entryOf(results.get("enforcement"))).toBe(
      join(parent, "regimen-enforcement", "src", "cli", "index.ts"),
    );
  });
});
