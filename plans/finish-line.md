...
- 2026-08-20: caught the log up after seven weeks of work that landed without entries here. Stage 1 is merged; `feat/judge-prompt-setup-rubric` is gone and ADR-0017 is adopted on main. A2 and A3 are done: the unified `regimen` CLI ships both the external-key judge and the no-key CLI judge, and all four harnesses capture into one store on Linux, macOS and native Windows. A8 is done: `regimen status` prints the awaiting-assessment count. B6 is done: the three sibling repos are archived read-only and the local clones moved to ~/dev/archive/. A1 is part-built, the four-category taxonomy is designed in docs/regimen-categories-of-improvement.md with the adversarial pass and the taxonomy test still open. A4 is in progress by hand rather than as one metered sweep: the backlog is down to 126 conversations from 168, and 146 conversations are permanently unjudgeable because their transcripts aged out before anyone judged them.
- 2026-08-20: that last fact produced work this plan never anticipated. ADR-0015 gave the corpus a bulk sweep but left it dependent on someone remembering to run it, and an unjudged conversation becomes an unjudgeable one on the harness's own transcript-deletion schedule. ADR-0018 closes that window: the capture daemon triggers a nightly sweep, bounded by quiescence and a coverage watermark. Built and open as PR #81. It is a prerequisite for A4 being meaningful rather than a one-off, so treat it as A4's other half.
- 2026-08-20: filed #82, a dispatch test can reach the real judge endpoint and spend a metered call, because `runCli` loads ~/.config/regimen/env before it dispatches. Found the expensive way.
