...
Phase 0 (local validation) is done; its results are recorded below as verified facts, so they do not need re-deriving. Phase 1.1 to 1.3 are also done and live-validated as of 2026-06-02: the Codex capture hook (producer) and the translator (consumer) are built, unit- and smoke-tested, and confirmed end-to-end against a real `codex exec` session (details in 1.2 and 1.3). The immediate next task is Phase 1.4: the rollout-file tailer, the version-proof fallback and the judge-time transcript source, which reuses the already-built evidence layer. Hook-based capture stays the primary real-time path.

Status (2026-06-02): the 1.2/1.3 code lives on branch `feat/codex-capture` in `regimen-feedback`, three commits (a loader-test deflake, the Codex capture feature, and the removal of the repo-local hooks config). This plan's updates live on branch `docs/codex-trial-1.2-1.3` in the hub. Neither branch is pushed and no PR is open yet. A CodeRabbit review of the feedback branch came back clean (0 findings). `bun run check` is green (182 tests).
...
- `regimen-feedback`: `/home/mlo/dev/niftymonkey/regimen-feedback` (Bun). The Feedback instrument; build Codex capture here. Verify paths on open, but as of 2026-06-02:
  ...
  - Translators: `src/loader/translators/claude.ts` and `src/loader/translators/codex.ts` (done) beside it. v1 validation: `src/loader/translators/v1.ts`.
...
- [x] 0.4 Rollout format and fixtures. Taxonomy captured above. Hook-envelope fixtures for translator TDD are done (`samples/codex-envelopes.jsonl`). Still pending for 1.4: save a couple of redacted ROLLOUT fixtures (the `rollout-*.jsonl` shape, distinct from hook envelopes) into the build repo for the tailer's tests; real rollouts are on disk in the active WSL2 home at `~/.codex/sessions/2026/06/02/`.
...
- [ ] 1.4 Rollout-file tailer (fallback and judge transcript source). Polling reader of `CODEX_HOME/sessions/**/*.jsonl` per the rollout taxonomy (see the "Rollout JSONL format" section). Pairs tool spans by `call_id` (NOT `tool_use_id`, which is the hook key). Needs rollout fixtures (see 0.4); real rollouts are on disk in the active WSL2 home `~/.codex/sessions/2026/06/02/`. It feeds the same v1 events the hook translator already emits, so it can reuse the `codex` translator's mapping vocabulary; it is also where the missing `session.end` boundary and per-file churn (from `apply_patch`/`exec_command_end`) come from. Lower priority than hooks; also serves judge-time transcript reading (issue #14).
