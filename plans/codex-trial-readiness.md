...
- [x] 1.2 Codex capture hook (producer). `hooks/capture-codex.ts` (a self-contained near-copy of `hooks/capture.ts` stamping `harness: "codex"`, per the option-B file-per-harness decision), plus a repo-local `.codex/hooks.json` wiring `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `PreCompact` (no matcher, no `statusMessage`) via the docs' `$(git rev-parse --show-toplevel)` path. Built TDD (`tests/capture-codex.test.ts`): codex stamp, enabled-flag gate, capture-failure-never-surfaces. LIVE-validated 2026-06-02: an isolated `codex exec` run (temp `CODEX_HOME`, copied auth, inline `[hooks]` in `config.toml`) fired `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse` and wrote codex envelopes to the buffer; all dispatched through the translator with 0 quarantines. The shell tool reports `tool_name: "Bash"` with a `call_...` `tool_use_id` shared across the pre/post pair. Caveat: when both a user-level hook source and the repo-local `.codex/hooks.json` are present and the project is trusted, Codex loads both, so capture double-fires (the trial's user-level-only install in arbitrary repos avoids this).
