...
## 5. The purity contract on every prompt builder (load-bearing for external eval)

Every prompt-construction function in `packages/feedback/src/judged/` (`buildJudgePrompt` in `prompt.ts`, `buildRollupPrompt` in `rollup.ts`, `buildAuditSynthesisPrompt` in `audit-synthesis.ts`) is a pure, idempotent function of its structured-data argument alone: given the same chunks, setup, header, verdicts, or report, it returns byte-identical `{ system, user }` text every time. No clock read, no environment read, no store or network access, no randomness inside the builder. Time and versions enter only as explicit parameters (or, for `buildJudgePrompt`, as the pinned `PROMPT_VERSION`/`RUBRIC_VERSION` constants the caller stamps onto the envelope separately, never read inside the builder itself). Anything impure a call site needs (the clock for `generatedAt`, the store for reading conversations, the model port for the actual completion) lives in the caller, one layer up from the builder.

This is what lets an eval framework (evalite or equivalent) import a builder directly, feed it fixture data, and get the exact prompt Regimen would have sent, with no incidental drift from wall-clock time, locale, or ambient environment. Each builder is covered by an idempotence test (two calls, same input, `toEqual`) and a purity-guard test (a fixed input hashed with sha256, asserting the hash is stable) in the package's test suite, rather than a brittle full-text snapshot. A new prompt builder joining this module keeps the same contract: pure over data, versions and clock passed in, never read.
